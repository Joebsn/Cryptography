...
public class Blowfish {
	...
	private String binToHex(String plainText) {
		...
		while (hexa.length() < (plainText.length() / 4))
			// maintain output length same length as input by appending leading zeroes.
			hexa = "0" + hexa;

		return hexa;
	}

	private String xor(String a, String b) // xor two hexadecimal strings of the same length
	{
		a = hexToBin(a);
		b = hexToBin(b);
		String ans = "";
	...
	private String addBin(String a, String b) // addition modulo 2^32 of two hexadecimal strings.
	{
		String ans = "";
	...
	private String f(String plainText) {
		...
		String ans = "";
		for (int i = 0; i < 8; i += 2) {
			// the column number for S-box is 8-bit value(8*4 = 32 bit plain text)
	...
	private void keyGenerate(String key) // generate subkeys.
	{
		int j = 0;
		for (int i = 0; i < P.length; i++) // xor-ing 32-bit parts of the key with initial subkeys.
		{
			P[i] = xor(P[i], key.substring(j, j + 8));
			System.out.print("subkey " + (i + 1) + ": " + P[i] + "\t");
			if ((i + 1) % 4 == 0)
				System.out.println();
			j = (j + 8) % key.length();
		}
	...
	private String round(int time, String plainText, boolean newLine, boolean encryption) {
		String left, right;
		...
		if (encryption)
			System.out.print("round " + time + ": " + right + left + "\t");
		else
			System.out.print("round " + (time - 2) + ": " + right + left + "\t");
	...
	private String encrypt(String plainText) {
		...
		for (int i = 0; i < 16; i++) {
			if ((i + 1) % 4 == 0)
				newLine = true;
			plainText = round(i, plainText, newLine, true);
			newLine = false;
		}
	...
	private String decrypt(String plainText) {
		...
		for (int i = 17; i > 1; i--) {
			if ((i + 1) % 4 == 0)
				newLine = true;
			plainText = round(i, plainText, newLine, false);
			newLine = false;
		}
	...
	private String WordToHexa(String word) {
	...
	private String HexaToWord(String hexa) {
	...
	public String convertPlaintextToCiphertext(String plainText) {
		List<String> list = new ArrayList<String>();

		System.out.println("\n-----Plaintext-----");
		System.out.println("Plaintext: " + plainText + "\n");

		String hexadecimal = WordToHexa(plainText);
		System.out.println("-----Hexadecimal-----");
		System.out.println("Hexadecimal: " + hexadecimal + "\n");

		if (hexadecimal.length() == 16)
			list.add(hexadecimal);
		else if (hexadecimal.length() < 16) {
			while (hexadecimal.length() < 16)
				hexadecimal += '0';
			list.add(hexadecimal);
		} else {
			int i = 0;
			while (i + 16 < hexadecimal.length()) {
				list.add(hexadecimal.substring(i, i + 16));
				i += 16;
			}
			if (i < hexadecimal.length()) {
				String word = hexadecimal.substring(i, hexadecimal.length());
				while (word.length() < 16)
					word += '0';
				list.add(word);
			}
		}
		System.out.println("-----Hexadecimal (Each of length 16) with zero padding if needed-----");
		list.stream().forEach(System.out::println);
		System.out.println();
		String completeCiphertext = "";
		for (int i = 0; i < list.size(); i++) {
			hexadecimal = list.get(i);
			System.out.println("-----Encryption-----");
			String cipherText = encrypt(hexadecimal);
			System.out.println("Cipher Text: " + cipherText + "\n");
			completeCiphertext += cipherText;
		}

		System.out.println("\n-----Complete Ciphertext-----");
		System.out.println("Complete Ciphertext: " + completeCiphertext);
		return completeCiphertext;
	}

	public String convertCiphertextToPlaintext(String completeCiphertext) {
		System.out.println("-----Complete Ciphertext Received-----");
		System.out.println("Complete Ciphertext Received: " + completeCiphertext + "\n");

		// Received Ciphertext will splitted it into words of length 16
		List<String> list = new ArrayList<String>();
		int j = 0;
		while (j < completeCiphertext.length()) {
			list.add(completeCiphertext.substring(j, j + 16));
			j += 16;
		}

		System.out.println("-----Hexadecimal with zero padding received on the server side-----");
		list.stream().forEach(System.out::println);
		System.out.println();

		String plaintextreceived = "";
		for (int i = 0; i < list.size(); i++) {
			String cipherText = list.get(i);
			System.out.println("-----Decryption-----");
			String newHexadecimal = decrypt(cipherText);
			System.out.println("\nHexadecimal: " + newHexadecimal);
			String newplainText = HexaToWord(newHexadecimal);
			System.out.println("Plaintext: " + newplainText + "\n");
			plaintextreceived += newplainText;
		}

		System.out.println("\n-----Complete Plaintext-----");
		System.out.println("Complete Plaintext: " + plaintextreceived);
		return plaintextreceived;
	}
	...
	public String reverseString(String toReverse) {
		...
		for (int i = 0; i < toReverse.length(); i++) {
			ch = toReverse.charAt(i);
			nstr = ch + nstr;
		}
		return nstr;
	}

	public String getStringKey(double number) {
		String key = "", n = Double.toString(number), numberAfterExponential = "";
		for (int i = n.length() - 1; i >= 0; i--) {
			Character ch = n.charAt(i);

			if (ch.equals('E'))
				break;
			numberAfterExponential += ch;
		}
		numberAfterExponential = reverseString(numberAfterExponential);
		System.out.println("Number after exponential is: " + numberAfterExponential);
		...
		int lengthOfNumber = Integer.parseInt(numberAfterExponential);
		while (j < lengthOfNumber)
			key += "10";
		return key;
	}

	public String createLettersInKey(String key) {
		String newKey = "";
		char ch = 'a';
	...
	Blowfish(double key) {
		...
		for (int i = 0; i < 32; i++)
			modVal = modVal << 1; // (<<1 is equivalent to multiply by 2)

		System.out.println("key in double format is " + Double.toString(key));
		String keyy = getStringKey(key);
		System.out.println("key in string format is " + keyy);
		System.out.println("Key length is " + keyy.length());
		System.out.println("Converting the key to hexadecimal to become of length 16");
		String newKey = WordToHexa(keyy);
		newKey = createLettersInKey(newKey);
